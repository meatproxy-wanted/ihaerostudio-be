...
참조가 있는 장면은 **Qwen-Image-Edit-2511 (FP8, 20 steps, CFG 4)**로 편집 생성합니다.
...
최초 인물 초상은 **Qwen-Image-2512 (FP8, 512×512, 20 steps, CFG 4)**, 참조 없는 장면은 **FLUX.2 Dev (512×512, 20 steps)**를 사용합니다.
...
강한 2D 제약과 입체 표현·명암·그라데이션 금지 목록은 제거했습니다. 모든 새 그림은 부드러운 손그림·만화 일러스트로 지시하고 가벼운 명암은 허용합니다.
사진·실사 피부·모공 표현을 피하도록 지시하며 Qwen 초상·Edit의 negative prompt도 짧은 사진 표현 제외 목록으로 바꿨습니다. 참조 인물의 얼굴·머리·옷·색은 유지합니다.
...
Qwen 출력은 첫 참조의 비율을 약 1MP로 정규화한 뒤 가로·세로를 50%로 축소하며 정사각형 참조이면 512×512입니다.
