...
새 초상과 장면은 번들 화풍 샘플을 실제 이미지로 넣은 Qwen Edit 40 steps를 사용합니다. 샘플은 분위기만 가볍게 참고하도록 지시하며 인물·구도를 복사하지 않습니다. 인물 외형·상황이 우선이고 수치형 스타일 강도는 없습니다. 새 초상은 512×512 빈 latent에서 생성하며 한 명·흰 배경 검사는 유지합니다.
...
샘플이 없는 구형 호환 경로는 초상 **Qwen-Image-2512 (FP8, 512×512, 20 steps, CFG 4)**, 참조 없는 장면 **FLUX.2 Dev (512×512, 20 steps)**입니다.
