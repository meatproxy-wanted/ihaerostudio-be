...
최초 인물 초상은 **Qwen-Image-2512 (FP8, 1328×1328, 20 steps, CFG 4)**, 참조 없는 장면은 **FLUX.2 Dev (1024×1024, 20 steps)**를 사용합니다.
모든 새 그림은 평면 2D(선·평면 도형·단색 채움)만 사용하도록 최종 생성 프롬프트에 지정합니다. 3D·2.5D·입체 명암·그림자·광택·그라데이션·클레이 질감은 제외합니다.
Qwen 초상·Edit의 negative prompt에도 입체 표현을 제외합니다. 참조 인물의 외형은 유지하되 입체 스타일은 복사하지 않습니다.
...
Qwen 출력은 첫 참조의 비율을 약 1MP로 정규화하며 정사각형 참조이면 1024×1024입니다.
