...
후보만 생성하고 저장하지 않은 그림은 사용하지 않습니다. 최대 6개의 PNG/JPEG/WebP 기준 그림을 지원합니다.
기준 그림은 OpenAI 이미지 입력으로 읽어 얼굴·머리·수염 유무·상의·하의·신발을 고정 외형으로 저장합니다. 같은 소유자·프로젝트·자산의 동일 픽셀은 분석 결과를 재사용합니다. 생성된 그림도 OpenAI가 관찰하고 서버가 수염·의상 등 항목별 차이를 검사합니다. 불일치 후보는 저장·반환하지 않습니다. 첫 불일치는 502 image_identity_mismatch이며 같은 요청을 다시 보내면 유료 보정 생성을 한 번 진행합니다. 보정도 실패하면 502 image_identity_unresolved로 멈추며 추가 자동 생성은 없습니다. 얼굴을 확인할 수 없거나 여러 사람이 있는 기준 그림은 422 character_reference_unclear입니다. AI 검사에는 오판 가능성이 있어 완벽한 외형 일치를 보장하지 않습니다.
기준이 없거나 등장인물 카드 자체를 생성할 때는 텍스트 기반 생성 후 얼굴 가시성을 검사합니다. 사람이 없는 장면은 허용하지만 인물 카드는 얼굴이 보이는 한 사람이어야 합니다. SVG·기준 충돌·개수 초과는 `422 character_reference_*`로 안내합니다.
