...
RESPONSE_SCHEMAS = {
    ...
    "StudioImagePreparation": obj("편집 진입 시 서버가 생성한 그림을 자동 적용한 최신 문서와 진행 상태입니다.",
        document=ref("EasyDocument"), project=ref("StudioProject"), generation=obj("동일 POST로 이어받는 일괄 생성 상태입니다.",
            status={"type": "string", "enum": ["running", "ready", "skipped"]},
            phase={"type": "string", "enum": ["portraits", "scenes", "complete"]},
            completed=INTEGER, total=INTEGER, currentCardId=nullable(ID))),
...
FIELD_DESCRIPTIONS = {
    ...
    "DocImage": {"src": "이 프로젝트에서 생성·업로드한 그림의 주소(/api/studio/assets/{id})와 같아야 합니다. 임의 외부 URL은 저장할 수 없습니다.", "alt": "그림의 실제 모습을 설명하는 대체텍스트. 비어 있으면 제작자가 직접 보완하며 현재 자동 검토 규칙에는 포함되지 않습니다.", "meaning": "그림으로 전달하려는 의미. 글의 뜻과 같은지 별도 대조합니다.", "source": "upload=직접 업로드, library=후보·자동 준비 자산. 고정 캐릭터 얼굴과 생성 장면 모두 library를 사용하므로 stock 캐릭터 여부가 아닙니다."},
