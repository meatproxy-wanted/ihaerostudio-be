...
RESPONSE_SCHEMAS = {
    ...
    "StudioImageCandidate": obj("선택 전 그림 후보. 문서에 붙일 때 FE에서 그림 ID를 부여하고 source=library로 구성합니다.",
        src=field(STRING, "보관한 이미지의 data URI. 생성 결과는 PNG이며 업로드 후보에는 안전한 SVG도 포함될 수 있습니다."),
    ...
    "StudioReviewItem": obj("required는 처리해야 검토 완료가 가능하고 suggested는 완료를 막지 않습니다.",
        ...
        category=field({"type": "string", "enum": ["numbers", "relations", "claim-mix", "image-meaning", "no-anchor", "structure-changed", "hard-term", "long-sentence", "alt-text"]}, "점검 유형. hard-term은 FE 계약에 있으나 현재 서버 규칙은 생성하지 않습니다."),
...
FIELD_DESCRIPTIONS = {
    ...
    "DocImage": {"src": "이 프로젝트에서 생성·업로드한 이미지와 동일한 data URI여야 합니다. 임의 외부 URL은 저장할 수 없습니다.", "alt": "그림의 실제 모습을 설명하는 대체텍스트. 비어 있으면 검토 항목이 생성될 수 있습니다.", "meaning": "그림으로 전달하려는 의미. 글의 뜻과 같은지 별도 대조합니다.", "source": "upload=직접 업로드, library=후보에서 선택. 현재 생성 그림 후보도 FE 계약에 맞춰 library로 연결합니다."},
