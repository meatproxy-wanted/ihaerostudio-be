...
오른쪽 **Authorize**에는 백엔드의 **제작자 API 토큰 값만** 입력합니다. Swagger가
`Authorization: Bearer <토큰>` 헤더를 붙입니다. 로컬 기본 토큰은 `dev-only-change-me`이며 운영 환경에서는 사용할 수 없습니다.
이 토큰은 `API_KEYS`에 등록된 작성자를 식별합니다. **OpenAI 키나 Comfy 키를 Authorize에 넣지 마세요.**
별도 회원가입·로그인·토큰 발급 API는 없고, AI 제공자 키는 서버 환경변수로만 관리합니다.
...
텍스트 100~100,000자, PDF 20MB·100쪽·추출 텍스트 150,000자·문단 300개, 그림 업로드 2MB,
...
ERRORS = {
    "unauthorized": (401, "유효한 Bearer 토큰이 필요합니다.", "Authorize의 제작자 토큰과 서버 API_KEYS 설정을 확인합니다."),
    ...
    "request_too_large": (413, "요청은 최대 22MB입니다.", "HTTP 요청 본문 전체를 줄입니다. 파일 자체 한도는 이보다 작을 수 있습니다."),
    ...
    "file_too_large": (413, "PDF는 최대 20MB입니다.", "20MiB 이하의 PDF로 나눠 올립니다."),
