...
| 실제 AI 모드 + `COMFY_CLOUD_API_KEY` + 그림 후보 보기 | OpenAI가 저장된 카드의 장면 설명을 만들고 Comfy가 새 초상과 장면을 화풍 샘플을 참고하는 Qwen-Image-Edit-2511 (FP8, 40 steps, CFG 4)로 생성합니다. 초상은 768×768이고 인물 참조 장면은 첫 참조 비율을 유지하며 정사각형이면 768×768입니다. 인물 참조가 3개면 별도 샘플은 생략합니다. |
...
describe("prepare_images", "편집 진입 그림 일괄 생성 — 인물 먼저 생성·고정 후 장면 자동 적용",
    "편집 페이지 진입 직후 자동으로 호출합니다. 카드마다 사용자가 생성·선택할 필요가 없습니다. 장면은 초상과 별도로 핵심 행동·인물 배치/시선/손동작·사물 상태·주장/사실/명령/완료 구분을 한 번의 OpenAI 호출로 구조화하여 생성합니다. 장면에는 흰 배경을 강제하지 않으며 근거가 없는 장소·사건은 추가하지 않도록 지시합니다. 인물 초상은 한 명·빈 흰 배경 그대로이며 AI 호출 횟수는 유지하고 해상도는 768px 기준으로 올리며 화풍 샘플을 적용한 새 초상과 장면은 40 steps를 사용합니다. 기존 적용 그림과 완료된 일괄 작업은 자동 교체하지 않습니다. 상황 표현의 정확성을 보증하는 검사는 아닙니다.",
