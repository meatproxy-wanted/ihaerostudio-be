...
| 실제 AI 모드 + `COMFY_CLOUD_API_KEY` + 그림 후보 보기 | OpenAI가 저장된 카드의 장면 설명을 만들고 Comfy가 768×768 그림 1장을 생성합니다. |
...
ERRORS = {
    ...
    "character_reference_unclear": (422, "기준 그림에서 한 사람의 얼굴을 확인하기 어려워요.", "눈·코·입과 수염 유무를 확인할 수 있는 한 사람의 그림을 등장인물 카드에 적용하고 저장합니다."),
    "image_identity_mismatch": (502, "인물의 외형이 기준과 달라 후보에서 제외했어요.", "같은 요청으로 다시 시도하면 유료 보정 생성을 한 번 진행합니다. 실패 후보는 자산이나 후보로 반환하지 않습니다."),
    "image_identity_unresolved": (502, "보정 후에도 인물 외형이 기준과 달라 후보에서 제외했어요.", "추가 자동 생성은 멈춥니다. 기준 그림이나 장면 설명을 수정한 뒤 새 후보를 요청합니다."),
    ...
    "character_reference_limit": (422, "한 장의 그림에는 등장인물 기준 그림을 6개까지 사용할 수 있어요.", "문서의 등장인물 기준 그림을 최대 6개로 조정합니다. 서버가 임의로 기준 그림을 누락하지 않습니다."),
...
describe("image_candidates", "그림 후보 생성·조회 — OpenAI 장면 설명과 Comfy 그림",
    ...
    "먼저 등장인물(role=person, partyId 지정) 카드에서 그림을 생성·선택하고 PUT document로 저장하세요. 결론·주장·판단·배경 그림을 요청하면 서버가 document.images와 인물 카드의 imageId를 연결해 저장된 기준 그림을 찾습니다. 후보만 생성하고 적용하지 않은 그림은 기준으로 쓰지 않습니다.\n\nOpenAI에는 대상 문장·원문 근거와 등장인물의 이름·역할·저장된 설명, 기준 그림 번호를 함께 전달합니다. 누가 누구에게 무엇을 해야 하는지와 주장·판단·결정을 구분해 장면을 설계합니다. 기준 그림은 OpenAI 이미지 입력으로 읽어 얼굴·머리·수염 유무·상의·하의·신발을 고정 외형으로 저장합니다. 같은 소유자·프로젝트·자산의 동일 픽셀은 분석 결과를 재사용합니다. 생성된 그림도 OpenAI가 관찰하고 서버가 수염·의상 등 항목별 차이를 검사합니다. 불일치 후보는 저장·반환하지 않습니다. 첫 불일치는 502 image_identity_mismatch이며 같은 요청을 다시 보내면 유료 보정 생성을 한 번 진행합니다. 보정도 실패하면 502 image_identity_unresolved로 멈추며 추가 자동 생성은 없습니다. 얼굴을 확인할 수 없거나 여러 사람이 있는 기준 그림은 422 character_reference_unclear입니다. AI 검사에는 오판 가능성이 있어 완벽한 외형 일치를 보장하지 않습니다. 실제 기준 이미지 픽셀은 Comfy에 업로드하여 Flux.2 Klein 9B의 다중 이미지 입력으로 전달합니다. PNG·JPEG·WebP 최대 6개를 사용하며 외형·옷·색을 유지하도록 지시합니다. 그림은 768×768 한 장입니다. 인물 카드 자체를 생성할 때나 저장된 기준 그림이 없을 때는 Flux Schnell 생성 후 얼굴 가시성을 검사합니다. 뒷모습·옆모습·가려진 얼굴은 후보에서 제외합니다. 인물 카드는 얼굴이 보이는 한 사람이어야 하고, 일반 장면은 사람이 없어도 됩니다. 얼굴 검사 실패에도 image_identity_mismatch와 한 번의 보정 제한을 적용합니다. 외형 일치나 관계의 정확성을 자동 보증하지는 않습니다.\n\n같은 카드와 등장인물 기준은 저장/진행 작업을 재사용합니다. 인물 설명·역할·기준 그림을 바꾸면 다음 요청은 새 생성이 될 수 있고, 이전 결론 그림을 자동 교체하지 않습니다. 데모는 업로드 후보만 반환합니다. 후보는 프로젝트 자산으로 보관하지만 문서에 자동 연결하지 않습니다.\n\nComfy 완료 대기 90초 후 image_in_progress이면 같은 요청으로 이어서 확인합니다. 접수 불확실·실행 실패·만료는 새 유료 작업을 자동 제출하지 않습니다. 생성 중 카드나 등장인물 기준이 바뀌면 image_card_changed로 오래된 후보 적용을 막습니다. 브라우저 요청 취소가 제공자 작업 취소를 뜻하지는 않습니다.",
    ...
    errors="not_found image_limit image_too_large invalid_image unsupported_image image_card_changed character_reference_invalid character_reference_ambiguous character_reference_limit character_reference_unclear image_identity_mismatch image_identity_unresolved comfy_not_configured comfy_auth_error comfy_insufficient_credits comfy_rate_limited comfy_workflow_unavailable image_in_progress image_submission_unknown comfy_connection_or_response_error comfy_invalid_response comfy_asset_host_not_allowed comfy_download_failed image_missing_output image_generation_failed " + AI_ERRORS,
