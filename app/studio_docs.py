...
| 실제 AI 모드 + `COMFY_CLOUD_API_KEY` + 그림 후보 보기 | OpenAI가 저장된 카드의 장면 설명을 만들고 Comfy가 등장인물 초상은 Qwen-Image-2512 (FP8, 512×512, 20 steps, CFG 4), 참조 없는 장면은 FLUX.2 Dev (FP8, 512×512, 20 steps), 참조 장면은 Qwen-Image-Edit-2511 (FP8, 20 steps, CFG 4)로 그림 1장을 생성합니다. |
...
describe("prepare_images", "편집 진입 그림 일괄 생성 — 인물 먼저 생성·고정 후 장면 자동 적용",
    "편집 페이지 진입 직후 자동으로 호출합니다. 카드마다 사용자가 생성·선택할 필요가 없습니다. 장면은 초상과 별도로 핵심 행동·인물 배치/시선/손동작·사물 상태·주장/사실/명령/완료 구분을 한 번의 OpenAI 호출로 구조화하여 생성합니다. 장면에는 흰 배경을 강제하지 않으며 근거가 없는 장소·사건은 추가하지 않도록 지시합니다. 인물 초상은 한 명·빈 흰 배경 그대로이며 AI 호출 횟수·512px를 유지하며 화풍 샘플을 적용한 새 초상과 장면은 40 steps를 사용합니다. 기존 적용 그림과 완료된 일괄 작업은 자동 교체하지 않습니다. 상황 표현의 정확성을 보증하는 검사는 아닙니다.",
    ...
    "요청당 최대 카드 한 개를 생성·자동 저장하므로 Vercel 백그라운드 스레드에 의존하지 않습니다. 모든 인물 초상을 먼저 Qwen-Image-2512 (20 steps)로 만듭니다. 새 초상 설계에는 같은 자료의 다른 저장된 인물의 실제 외형 분석을 비교 데이터로 전달하여 얼굴형·머리 모양/색·상의 종류/색·안경 등에서 적어도 두 가지 특징이 다르게 하도록 지시합니다. 그림체는 유지하며 기존 인물을 함께 그리거나 편집 참조로 복사하지 않습니다. 비교 대상과 분석을 작업에 고정하고 픽셀 분석 캐시를 재사용하여 나중에 다른 인물이 완성되어도 기존 초상을 재생성하지 않습니다. 실제 외형 차이를 자동 보증하지 않습니다. 모든 새 그림의 스타일 요구는 단순한 애니메이션 이미지 또는 삽화 한 문장입니다. 외형 분석의 그림체 설명은 생성 지시로 전달하지 않고 참조 인물의 얼굴·머리·옷·색만 유지합니다. Qwen Edit의 negative prompt는 비워 두며 초상에는 인원수·배경·글자·얼굴 구도 조건만 남깁니다. 스타일 지시는 결과의 100% 보장이 아닙니다. 구형 완료 후보 캐시는 재사용하지 않으며 접수 전 그래프는 재구성하지만 기존 고정 그림과 이미 접수된 유료 작업은 유지합니다. 인물을 고정한 다음, 그 참조로 Qwen Edit 장면을 생성합니다. 초상은 한 명·빈 흰 배경 조건을 positive/negative prompt로 지정하고 저장 전 GPT 이미지 분석으로 검사합니다. 불통과하면 portrait_composition_invalid로 적용하지 않고 같은 요청에 자동 유료 재생성하지 않습니다. 검사 장애는 기존 생성 결과의 검사만 재시도하며 AI 검사는 정확성을 보장하지 않습니다. 인물 그림 교체·삭제는 서버에서도 막습니다. 완료된 일괄 작업을 반복 호출하거나 장면 그림을 나중에 제거해도 자동 재생성하지 않습니다. 글·장면·대체텍스트 편집과 제작자 검토는 유지합니다. 새로고침은 완료 카드를 건너뛰며 진행 중 Comfy 작업을 재조회합니다. 접수 불확실·실패는 유료 작업을 자동 재제출하지 않습니다. 초안 재생성은 인물 그림을 유지하고 새 장면의 일괄 작업을 초기화합니다.",
...
describe("image_candidates", "그림 후보 생성·조회 — OpenAI 장면 설명과 Comfy 그림",
    "편집기의 카드 → 그림 넣기/바꾸기 → 그림 후보 보기에서 호출합니다. 장면은 초상과 분리하여 핵심 행동·인물 배치/시선/손동작·사물 상태·의미 구분을 한 번의 OpenAI 호출로 구조화하고 생성 프롬프트에 반영합니다. 장면의 흰 배경 강제를 제거하고 원문 근거가 있는 상황만 표현하도록 지시합니다. 주장한 과거 사건을 확정 사실로 재현하거나 지급 명령을 지급 완료로 그리지 않습니다. 기준 인물 외형과 512px를 유지하며 화풍 샘플을 적용한 새 초상과 장면은 40 steps를 사용하고 기존 그림은 자동 교체하지 않습니다. 구형 완료 후보 캐시 대신 새 설계를 사용하지만 이미 접수되거나 접수 여부가 불확실한 유료 작업은 원래 그래프로 재제출 없이 조회합니다. 상황 표현의 정확성은 제작자가 실제 후보를 확인해야 합니다.", "JSON {cardId}를 보냅니다. 카드 문장·원문 근거·당사자 표시 이름은 저장된 문서에서 읽으므로 자동 저장 완료 후 요청하세요. 프롬프트·워크플로·jobId는 FE에서 보내지 않습니다.",
