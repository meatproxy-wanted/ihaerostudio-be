...
describe("prepare_images", "편집 진입 그림 일괄 생성 — 인물 먼저 생성·고정 후 장면 자동 적용",
    "편집 페이지 진입 직후 자동으로 호출합니다. 카드마다 사용자가 생성·선택할 필요가 없습니다. 장면은 초상과 별도로 핵심 행동·인물 배치/시선/손동작·사물 상태·주장/사실/명령/완료 구분을 한 번의 OpenAI 호출로 구조화하여 생성합니다. 장면에는 흰 배경을 강제하지 않으며 근거가 없는 장소·사건은 추가하지 않도록 지시합니다. 인물 초상은 한 명·빈 흰 배경 그대로이며 AI 호출 횟수·512px·20 steps를 유지합니다. 기존 적용 그림과 완료된 일괄 작업은 자동 교체하지 않습니다. 상황 표현의 정확성을 보증하는 검사는 아닙니다.",
