...
새 등장인물 초상과 장면은 번들 화풍 샘플 `app/assets/illustration-mood-v1.png`를 실제 이미지 입력으로 사용하는 **Qwen-Image-Edit-2511 (FP8), 40 steps, CFG 4**로 생성합니다. 새 초상은 512×512 빈 latent에서 시작해 샘플의 두 인물·구도를 바탕으로 삼지 않습니다.
...
구형 텍스트 초상 경로는 공식 기본 50 steps 대신 20 steps이며, 해상도는 512×512입니다. 새 샘플 초상은 Qwen Edit 40 steps입니다.
초상 픽셀 수는 기존 1328px 대비 약 85%, 일반 장면은 기존 1024px 대비 75% 감소합니다.
이전 768px 설정 대비 픽셀 수는 약 56% 감소하며 얼굴·표정 등의 세부 묘사가 줄 수 있습니다.
...
기준 그림이 없는 일반 장면은 **FLUX.2 Dev (FP8), 512×512, 20 steps**로 그림 1장을 생성합니다.
...
참조 그림은 FluxKontextImageScale로 약 1MP로 정규화한 뒤 ImageScaleBy(area, 0.5)로 가로·세로를 50%로 축소합니다.
축소한 같은 이미지 슬롯을 positive/negative encoder와 첫 참조 VAEEncode에 전달합니다.
출력은 첫 참조의 비율을 유지한 약 0.26MP이며 정사각형 기준 512×512입니다. 완성 후 파일만 줄이는 방식이 아닙니다.
