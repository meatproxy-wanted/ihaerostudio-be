...
등장인물 초상(`role=person`)은 **Qwen-Image-2512 (FP8), 768×768, 20 steps, CFG 4**로 생성합니다.
모든 인물·일반 장면·참조 편집 장면에 평면 2D 일러스트 스타일을 강제하는 최종 생성 지시를 붙입니다.
선명한 선·평면 도형·단색 채움만 사용하고 3D·2.5D·CGI·클레이·장난감 질감·입체 명암·그라데이션·그림자·광택·실사 조명은 제외합니다.
Qwen 초상과 Edit에는 같은 입체 표현을 negative prompt에도 지정합니다. FLUX.2 일반 장면은 기존 BasicGuider의 positive conditioning에 지정합니다.
참조 이미지의 외형·옷·색은 유지하되 입체적인 스타일은 복사하지 않고 평면 2D로 표현하도록 지시합니다.
구형 완료 후보 캐시는 새 2D 생성 요청에 재사용하지 않지만, 이미 적용·고정한 그림은 유지합니다.
...
서버 설정은 공식 기본 50 steps 대신 20 steps이며, 초상 해상도는 1328×1328에서 768×768로 낮췄습니다.
초상 픽셀 수는 기존 1328px 대비 약 67%, 일반 장면은 기존 1024px 대비 약 44% 감소합니다.
