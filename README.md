...
강한 2D 제약(평면 도형·단색 채움만 사용, 3D·2.5D·그라데이션·그림자·명암 금지)은 제거했습니다.
모든 새 인물·일반 장면·참조 편집 장면은 부드러운 손그림·만화 일러스트, 간결한 가상 인물·선·색을 중심으로 지시하고 가벼운 명암은 허용합니다.
사진·실사 피부·모공 표현을 피하도록 positive prompt에 지정하고 Qwen 초상과 Edit는 짧은 사진 표현 negative prompt를 사용합니다.
참조 이미지의 얼굴·머리·옷·색은 유지하면서 손그림 만화 인물로 표현하도록 지시합니다.
구형 완료 후보 캐시는 새 스타일 생성 요청에 재사용하지 않지만, 이미 적용·고정한 그림은 유지합니다.
접수 전 구형 그래프는 새 스타일로 재구성하며, 접수됐거나 접수 여부가 불확실한 유료 작업은 원래 그래프로 이어 조회합니다.
...
초상은 여전히 한 명·빈 흰 배경이며 고정된 등장인물은 변경하지 않습니다. AI 호출 횟수·해상도·steps는 늘리지 않습니다.
