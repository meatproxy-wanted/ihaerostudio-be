...
등록 키가 없는 익명 모드는 `API_KEYS={}`이며 개발용 공용 키를 기본 등록하지 않습니다.
keys 모드는 등록 키가 필수이고, 공개된 `dev-only-change-me`는 등록·익명 인증 모두에서 사용할 수 없습니다.
`APP_ENV=production`에서는 익명 모드의 등록 키도 32자 이상이어야 합니다.
현재 FE의 `NEXT_PUBLIC_STUDIO_API_TOKEN`은 브라우저에 노출되므로 비밀 관리자 키를 넣지 않습니다.
독자·그림 파일 조회는 인증이 없고 나머지 업무 API는 제작자 토큰이 필요합니다.

### 선택적 AI 호출 제한

`AI_CALLS_PER_HOUR`는 작업함별, `AI_CALLS_GLOBAL_PER_HOUR`는 서버 전체 한도입니다. 기본값 `0`은 제한 없음입니다.
GPT 외부 호출과 새 Comfy 제출 시도를 각각 1회로 세며, 실패한 시도도 집계합니다. 기존 Comfy 조회·다운로드와 완료 캐시 반환은 제외합니다.
여러 서버 인스턴스가 공유 DB의 카운터를 원자적으로 갱신하며 UTC 정시마다 초기화됩니다.
한도에 도달하면 새 호출만 `429 ai_call_limit`과 `Retry-After`로 거절합니다. 미제출 이미지 작업은 prepared 상태를 유지해 이후 재개합니다.
이는 호출 수 제한이지 실제 금액·토큰 사용량의 상한은 아닙니다. 익명 토큰을 바꾸면 개인 제한을 우회할 수 있으므로 운영에서는 서버 전체 한도도 함께 설정하세요.
HTTP 인증 작업에 적용하며 CLI의 직접 API 호출은 이 정책 밖입니다. OpenAI/Comfy 계정의 비용 관리도 별도로 필요합니다.
OpenAI의 사용자별 사용량 제한 권고는 [공식 문서](https://developers.openai.com/api/docs/guides/rate-limits)를 참고하세요.
