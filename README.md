...
`STUDIO_CHARACTER_MODE=generate`는 새 자료에서 기존 Qwen 초상 생성을 사용하는 호환 모드입니다. 이미 캐릭터셋을 배정한 자료는 모드를 바꿔도 배정을 유지합니다.
아래 초상 생성·화풍 샘플 설명은 generate 및 기존 자료 호환 경로에 해당합니다.
...
새 등장인물 초상과 장면은 번들 화풍 샘플 `app/assets/illustration-mood-v1.png`를 실제 이미지 입력으로 사용하는 **Qwen-Image-Edit-2511 (FP8), 40 steps, CFG 4**로 생성합니다. 새 초상은 768×768 빈 latent에서 시작해 샘플의 두 인물·구도를 바탕으로 삼지 않습니다.
샘플은 화면의 전반적인 분위기만 가볍게 참고하도록 지시하며, 인물·얼굴·옷·자세·배경·구도는 복사하지 않습니다. 인물 외형과 사건 상황이 우선입니다. 수치형 스타일 강도 기능이 아니라 프롬프트로 조절하므로 실제 영향은 생성 결과로 확인해야 합니다.
캐릭터 참조를 먼저, 샘플을 마지막 이미지 슬롯에 넣습니다. 장면에 기준 인물 3명이 필요하면 세 슬롯 모두 인물에 사용하고 별도 샘플은 생략합니다. 샘플 픽셀 digest와 정책 버전을 새 작업 캐시 키에 포함합니다. 기존 고정 인물·적용 그림·완료 일괄 작업은 교체하지 않습니다.
아래 Qwen-Image-2512/FLUX 경로는 샘플이 없는 구형 작업 및 호환 경로 설명입니다.
모든 새 인물·일반 장면·참조 편집 장면의 스타일 요구는 **단순한 애니메이션 이미지 또는 삽화** 한 문장입니다.
참조 이미지의 얼굴·머리·옷·색은 인물 외형 정보로 유지하지만 외형 분석의 그림체 설명은 생성 지시로 전달하지 않습니다.
Qwen Edit의 negative prompt는 비워 두며 초상의 negative prompt에는 인원수·배경·글자·얼굴 구도 조건만 남깁니다.
구형 완료 후보 캐시는 새 스타일 생성 요청에 재사용하지 않지만, 이미 적용·고정한 그림은 유지합니다.
접수 전 구형 작업은 장면 설명부터 새 지시로 재설계하며, 접수됐거나 접수 여부가 불확실한 유료 작업은 원래 그래프로 이어 조회합니다.
스타일 지시는 결과를 100% 보장하는 검사가 아니며 자동 유료 재생성을 추가하지 않습니다.
...
기존 인물을 새 초상에 함께 넣거나 편집 참조로 복사하지 않습니다. 새 초상에는 화풍 샘플만 이미지 입력으로 사용합니다.
...
구형 텍스트 초상 경로는 공식 기본 50 steps 대신 20 steps이며, 해상도는 768×768입니다. 새 샘플 초상은 Qwen Edit 40 steps입니다.
초상과 참조 없는 장면은 768×768이며, 참조 장면은 첫 인물 참조의 비율을 유지한 약 0.59MP(정사각형이면 768×768)입니다.
512px 설정 대비 픽셀 수가 2.25배로 늘어납니다. 40 steps와 화풍·레퍼런스 설정은 그대로이며 생성 시간과 GPU 사용량은 늘 수 있습니다.
CFG와 스타일·인원수 조건은 유지하며 실제 속도·품질은 생성 결과로 확인해야 합니다.
접수 전 구형 초상 그래프는 20 steps로 재구성하지만, 이미 접수됐거나 접수 여부가 불확실한 작업은 중복 과금 없이 기존 설정으로 이어 조회합니다.
기준 그림이 없는 일반 장면은 **FLUX.2 Dev (FP8), 768×768, 20 steps**로 그림 1장을 생성합니다.
장면 설계는 초상 설계와 분리합니다. OpenAI 한 번의 호출에서 핵심 행동, 인물 배치·손동작·시선,
관련 사물의 위치·상태·배경, 주장/사실/명령/완료 구분을 구조화하고 실제 생성 프롬프트 앞부분에 반영합니다.
장면에는 빈 흰 배경을 강제하지 않으며, 원문 근거에 있는 상황만 필요한 배경과 행동으로 표현하도록 지시합니다.
장소가 없으면 중립적인 공간을 쓰고, 주장한 과거 사건을 확정 사실로 재현하거나 지급 명령을 지급 완료로 그리지 않습니다.
모든 카드를 판사·저울·인물 나열로 대체하지 않도록 카드별 행동 중심 구도 예시를 제공합니다.
초상은 여전히 한 명·빈 흰 배경이며 고정된 등장인물은 변경하지 않습니다. AI 호출 횟수·해상도는 유지합니다.
새 장면 요청에는 새 설계를 사용하지만 이미 적용한 그림이나 완료된 일괄 작업은 자동 교체하지 않습니다.
접수 전 구형 장면은 재설계하고 이미 접수되거나 접수 여부가 불확실한 작업은 기존 그래프로 재제출 없이 조회합니다.
구도 지시는 결과의 상황 표현·정확성을 보증하지 않습니다. 테스트는 외부 AI 모의 응답을 사용하므로 실제 그림 확인은 별도입니다.
등장인물 그림을 **적용·저장한 뒤**
결론·사건 설명 그림을 요청하면 **Qwen-Image-Edit-2511 (FP8), 40 steps, CFG 4**가
그 인물 그림을 실제 편집 입력으로 사용합니다. 인물 외형을 유지하고 자세·장면을 바꾸도록 지시합니다.
참조 장면은 [공식 사용 예제](https://huggingface.co/Qwen/Qwen-Image-Edit-2511)와 같은 40 steps입니다. 샘플을 사용하는 새 초상 및 인물 참조 없는 장면도 Qwen Edit 40 steps입니다.
20/40 비교용 내부 컴파일 옵션은 같은 레퍼런스·프롬프트·시드·크기·CFG를 유지하고 steps만 바꿉니다. FE API에서 steps를 받지는 않습니다.
접수 전 20-step 참조 그래프는 시드·장면 설명·업로드를 유지한 채 40 steps로 재구성합니다. 이미 접수되거나 접수 여부가 불확실한 작업은 기존 설정으로 조회합니다.
연산량과 대기 시간이 늘 수 있으며, 실제 생성 비교 전에는 상황·그림체 개선을 보증하지 않습니다.
참조 그림은 FluxKontextImageScale로 약 1MP로 정규화한 뒤 ImageScaleBy(area, 0.75)로 가로·세로를 75%로 축소합니다.
축소한 같은 이미지 슬롯을 positive/negative encoder와 첫 참조 VAEEncode에 전달합니다.
출력은 첫 참조의 비율을 유지한 약 0.59MP이며 정사각형 기준 768×768입니다. 완성 후 파일만 줄이는 방식이 아닙니다.
축소 노드는 [ComfyUI 공식 ImageScaleBy 구현](https://github.com/Comfy-Org/ComfyUI/blob/master/nodes.py)을 사용합니다.
