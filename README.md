...
기준 그림은 OpenAI 이미지 입력으로 읽어 얼굴·머리·수염 유무·상의·하의·신발을 고정 외형으로 저장합니다. 같은 소유자·프로젝트·자산의 동일 픽셀은 분석 결과를 재사용합니다. 생성된 그림도 OpenAI가 관찰하고 서버가 수염·의상 등 항목별 차이를 검사합니다. 불일치 후보는 저장·반환하지 않습니다. 첫 불일치는 502 image_identity_mismatch이며 같은 요청을 다시 보내면 유료 보정 생성을 한 번 진행합니다. 보정도 실패하면 502 image_identity_unresolved로 멈추며 추가 자동 생성은 없습니다. 한 사람을 특정할 수 없는 기준 그림은 422 character_reference_unclear입니다. AI 검사에는 오판 가능성이 있어 완벽한 외형 일치를 보장하지 않습니다.
