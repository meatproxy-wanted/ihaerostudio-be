...
이미지 입력을 지원하는 OPENAI_MODEL이 필요합니다. 최초 기준 분석과 생성 결과 검사에 API 사용량·시간이 추가됩니다.
기준이 없는 생성과 등장인물 카드 자체 재생성에는 외형 일치 대신 얼굴 가시성을 검사합니다. 인물 카드는 얼굴이 보이는 한 사람이어야 합니다. 기존 그림은 자동 교체되지 않습니다.
