...
FE에서 **사건 구조 확인 → 초안 생성 → 편집 진입**하면 모든 카드의 그림을 자동 생성·적용합니다.
`POST /api/studio/projects/{id}/document/prepare-images`를 `generation.status`가
`ready` 또는 `skipped`일 때까지 이어 호출합니다. 요청당 최대 카드 한 개를 처리하고
`phase`, `completed`, `total`, `currentCardId`로 실제 진행률을 표시합니다.
완료·진행 상태를 DB에 저장하므로 새로고침해도 완료된 그림을 다시 생성하지 않습니다.
페이지를 떠나면 추가 카드 처리는 멈추며, 이미 접수한 원격 작업은 다음 진입에 이어 조회합니다.
demo 모드와 그림 없음 설정은 `skipped`이며 실제 그림을 만든 것으로 표시하지 않습니다.
등장인물을 먼저 자동 생성·저장하고 그 참조로 장면을 생성합니다.
인물 그림은 고정되어 교체·삭제할 수 없고, 직접 PUT document로 우회해도
409 `character_locked`입니다. 초안 재생성도 고정된 인물 그림을 유지합니다.
글·장면 그림·대체텍스트 편집과 제작자 검토는 그대로 유지합니다.
이후 장면만 바꿀 때 기존 **카드 → 그림 후보 보기 → 후보 선택 → 적용** 흐름을 사용합니다.
OpenAI가 카드의 문장과 근거로 영문 장면 설명·한국어 대체텍스트 초안을 만들고,
등장인물 초상(`role=person`)은 **Qwen-Image-2512 (FP8), 1328×1328, 20 steps, CFG 4**로 생성합니다.
각 그림은 정확히 한 명의 익명 성인을 빈 순수 흰 배경에 그리도록 지시합니다.
별도의 negative prompt로 여러 사람·복제·반사·콜라주·배경 장식을 배제합니다.
저장 전에 OpenAI 이미지 입력으로 실제 인원수와 빈 흰 배경 여부를 검사합니다.
검사 불통과는 422 `portrait_composition_invalid`이며 적용·고정하지 않습니다.
같은 요청은 거부 결과를 재사용하며 자동 유료 재생성하지 않습니다. 검사 장애 때는 기존 Comfy 결과의 검사만 재시도합니다.
이 AI 검사는 정확성을 보장하지 않습니다. 이미 고정된 인물은 바꾸지 않습니다.
새 초상을 설계할 때 같은 자료에 적용·저장된 다른 인물의 실제 외형 분석을 비교 데이터로 전달합니다.
각 기존 인물과 얼굴형·머리 모양/색·상의 종류/색·안경 등에서 적어도 두 가지 특징이 구별되도록 구체적인 영문 디자인을 지시합니다.
그림체는 유지하며 기존 인물을 새 초상에 함께 넣거나 편집 참조로 복사하지 않습니다. Qwen 텍스트 생성 경로를 유지합니다.
비교 대상·분석 결과는 작업에 고정하고 기존 픽셀 분석 캐시를 재사용합니다. 나중에 다른 인물이 완성돼도 이전 초상을 재생성하지 않습니다.
실제 외형의 차이를 자동 보증하거나 유료 재생성하지 않습니다. 다른 프로젝트·소유자의 인물은 사용하지 않습니다.
초상 워크플로우는 [Comfy 공식 Qwen-Image-2512 가이드](https://docs.comfy.org/tutorials/image/qwen/qwen-image-2512)의 일반 모델 경로를 사용합니다.
서버 설정은 공식 기본 50 steps 대신 20 steps입니다. 해상도와 CFG는 유지하며 실제 속도·품질은 생성 결과로 확인해야 합니다.
접수 전 구형 초상 그래프는 20 steps로 재구성하지만, 이미 접수됐거나 접수 여부가 불확실한 작업은 중복 과금 없이 기존 설정으로 이어 조회합니다.
...
이미 완료·적용한 그림은 보관하지만, 적용하지 않은 구형 Schnell 후보 캐시는 새 Qwen 초상 요청에 재사용하지 않습니다.
접수된 구형 작업은 재제출 없이 이어 확인하고, 인물 초상인 경우 결과 구도 검사 후 저장합니다.
...
등장인물 그림은 얼굴이 크게 보이는 상반신 초상을 기본으로 지시합니다.
관객 쪽을 향하도록 강제하지 않고, 얼굴이 가려지거나 잘리지 않아 눈·코·입을 식별할 수 있게 지시합니다.
...
이미지 입력을 지원하는 OPENAI_MODEL이 필요합니다. 생성한 초상의 구도 검사와 최초 기준 이미지 분석에 API 사용량·시간이 추가됩니다.
